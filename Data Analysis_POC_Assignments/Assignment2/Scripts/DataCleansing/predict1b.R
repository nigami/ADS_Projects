...
model1 <- na.omit(read.csv("sampleformat_zerosfilled_temp.csv"),stringsAsFactors = FALSE)
model1$Date <- NULL
...
lm.fit=lm(kwh~.-Account-Year+I(Hour^2)-Day+I(temp^2), data=model3)
summary(lm.fit)
...
lm.fit=lm(kwh~.-Account-Year+I(temp^2)+I(Hour^2)-Day, data=train)
summary(lm.fit)
...
testModel1$kwh <-  pred
testModel1$kwh[testModel1$kwh <= 0] <- 0
testModel1$kwh[testModel1$kwh <= 0] <- mean(testModel1$kwh)

write.csv(testModel1,"zerosfilled.csv",row.names = FALSE)


p = accuracy(pred, testModel$kwh)
p = t(p)
write.csv(p,"PerformanceMatrix_part2b.csv")
