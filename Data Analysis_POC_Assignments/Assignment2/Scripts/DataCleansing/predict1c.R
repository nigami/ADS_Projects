...
library(leaps)
library(lubridate)
model1 <- na.omit(read.csv("sampleformat_both_tempa.csv",stringsAsFactors = FALSE))
model1$Date <- NULL
#zoo package
library(zoo)
model1$kwh[model1$kwh == 0] <- NA

cz <- zoo(model1$kwh)
model1$kwh <- na.locf(cz)
write.csv(model1,"sampleformat_zerosfilled_temp3a.csv",row.names = FALSE)
model1 <- na.omit(read.csv("sampleformat_zerosfilled_temp3a.csv",stringsAsFactors = FALSE))

#write.csv(model1,"test.csv",row.names = FALSE)
...
p = t(p)
write.csv(p,"PerformanceMatrix_part3a.csv")
