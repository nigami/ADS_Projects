...
nodata = na.omit(nodata)
for(i in 1:nrow(nodata)) {
  if((nodata$kwh[i]) < (mean(nodata$kwh) - (1.5)*sd(nodata$kwh)) &&
     (nodata$kwh[i]) > (mean(nodata$kwh) + (1.5)*sd(nodata$kwh))) {
    nodata$kwh[i] <- nodata$kwh[i-1];
  }
}

#row_sub = apply(nodata[c(3)],1,function(z) !any(z==0))
#write.csv(nodata[row_sub,],"Part1/nonzeroa.csv",row.names = FALSE)
write.csv(nodata,"Part1/botha.csv",row.names = FALSE)
